...
The processed datasets are stored in the data/ directory. Before running experiments, extract all files and subfolders contained within the Guangdong or California folder from the archive directly to the project root directory at the same level as the code/ folder.
...
### Inference
Inference uses the same shell scripts as training, and you only need to change the parameter from ```--is_training 1``` to ```--is_training 0```
